...
export const addToCart = async (req, res) => {
  try {
    ...
    if (req.user.role === 'farmer' && product.farm) {
      const ownFarm = await Farm.findOne({ ownerId: req.user._id });
      if (ownFarm && ownFarm._id.toString() === product.farm._id.toString()) {
        return res.status(400).json({ msg: 'You cannot purchase your own products' });
      }
    }
