...
// Calculate certification score from certifications array
export const calculateCertificationScore = (certifications = []) => {
  ...
  certifications.forEach(cert => {
    ...
    if (CERT_SCORES[certName]) {
      maxScore = Math.max(maxScore, CERT_SCORES[certName]);
    } else {
      // Try partial match
      for (const [key, score] of Object.entries(CERT_SCORES)) {
        if (certName.toLowerCase().includes(key.toLowerCase()) || 
            key.toLowerCase().includes(certName.toLowerCase())) {
...
export const updateFarmProfile = async (req, res) => {
  ...
  let farm = await Farm.findOne({ ownerId });
    ...
    // Calculate certification score if certifications are being updated
    let certificationScore;
    if (certifications !== undefined) {
      certificationScore = calculateCertificationScore(certifications);
    }
    ...
    const update = {
      $set: {
        ...
        ...(certificationScore !== undefined ? { certificationScore } : {}),
        ...(images ? { images } : {})
      },
      $setOnInsert: { ownerId }
    };
...
/**
 * Get nearby farms - CORRECT IMPLEMENTATION
 * Uses MongoDB $geoNear with certification-based ranking
 * MongoDB computes distance and ranking score
 */
export const getNearbyFarms = async (req, res) => {
  try {
    ...
    // Aggregation pipeline - $geoNear MUST be first stage
    const pipeline = [
      {
        $geoNear: {
          near: {
          ...
          distanceField: 'distance', // MongoDB computes distance
      ...
      // Compute final ranking score in DB
      {
        $addFields: {
          // Ranking formula: certificationScore - (distance / 100)
          // Higher certified farms rank higher, closer farms rank higher
          finalScore: {
            $subtract: [
      ...
      // Sort by finalScore (DESC) - best farms first
      {
        $sort: { finalScore: -1 }
      },
      // Limit results
    ...
    console.error('Get nearby farms error:', err);
