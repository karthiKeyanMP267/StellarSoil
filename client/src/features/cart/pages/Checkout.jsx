    ...
    try {
      ...
      const allItems = carts.flatMap(cart =>
        cart.items.map(item => ({
          ...
          farmId: cart.farm?._id || item.product.farm?._id || null, // Extract farmId
        }))
      );
      
      // Get the first available farmId from items (for single-farm checkouts)
      ...
      setCart({
        ...
        items: allItems,
        farmId: farmId, // Add farmId to cart
